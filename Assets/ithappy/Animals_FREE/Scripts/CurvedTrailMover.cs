...
public class CurvedTrailMover : MonoBehaviour
{
    ...
    private void DrawCurve() {
        if (!(lineRenderer = GetComponent<LineRenderer>())) { Debug.LogWarning("LineRenderer missing!"); return; }
        int s = 4; lineRenderer.positionCount = s; lineRenderer.useWorldSpace = true;
        Vector3 p0 = startPoint.position, p2 = endPoint.position, p1 = (p0 + p2) * 0.5f + controlPointOffset;
        for (int i = 0; i < s; i++) {
            float t = i / (s - 1f), u = 1 - t;
            lineRenderer.SetPosition(i, u * u * p0 + 2 * u * t * p1 + t * t * p2);
        }
    }
